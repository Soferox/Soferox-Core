...
const std::string strMessageMagic = "Soferox Signed Message:\n";
...
void static UpdateTip(const CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int i = 0; i < 100 && pindex != nullptr; i++)
        {
            ...
            if (pindex->nVersion != 112 && pindex->nVersion > VERSIONBITS_LAST_OLD_BLOCK_VERSION && (pindex->nVersion & ~nExpectedVersion) != 0)
                ++nUpgraded;
            pindex = pindex->pprev;
        }
        if (nUpgraded > 0)
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if((nVersion < 2 && nHeight >= consensusParams.BIP34Height) ||
       (nVersion < 3 && nHeight >= consensusParams.BIP66Height) ||
       (nVersion < 4 && nHeight >= consensusParams.BIP65Height))
