...
int CAddrInfo::GetTriedBucket(const uint256& nKey) const
{
    ...
    ss1 << nKey << vchKey;
    uint64_t hash1 = XCoin::HashSofero(XCoin::ConstBuf(ss1.begin(), ss1.end())).GetCheapHash();

    CDataStream ss2(SER_GETHASH, 0);
    std::vector<unsigned char> vchGroupKey = GetGroup();
    ss2 << nKey << vchGroupKey << (hash1 % ADDRMAN_TRIED_BUCKETS_PER_GROUP);
    uint64_t hash2 = XCoin::HashSofero(XCoin::ConstBuf(ss2.begin(), ss2.end())).GetCheapHash();
...
CAddrInfo CAddrMan::Select_(bool newOnly)
{
    ...
    if (!newOnly &&
       (nTried > 0 && (nNew == 0 || GetRandIntFast(2) == 0))) {
        // use a tried node
